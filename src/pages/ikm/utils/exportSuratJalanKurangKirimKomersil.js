...
export default function exportSuratJalanKurangKirimKomersil(delivery, details) {
  ...
  const rowsHtml = details.map((item, idx) => {
    ...
    const len = item.length_cm !== undefined && item.length_cm !== null ? parseFloat(item.length_cm) : (item.lengthCm ? parseFloat(item.lengthCm) : null);
    const wid = item.width_cm !== undefined && item.width_cm !== null ? parseFloat(item.width_cm) : (item.widthCm ? parseFloat(item.widthCm) : null);
    const sizeStr = (len && wid) ? `${len}m x ${wid}m` : '—';
    
    let area = item.area_m2 !== undefined && item.area_m2 !== null ? parseFloat(item.area_m2) : (item.areaM2 ? parseFloat(item.areaM2) : 0);
    if (!area && len && wid) {
      area = len * wid;
    }
    const formattedArea = area > 0 ? `${area.toFixed(2)} m²` : '—';
    ...
    return `
      <tr style="color: #1e293b; border-bottom: 1px solid #0f172a;">
        ...
        <td style="padding: 10px 16px; border-right: 1px solid #0f172a; text-align: center;">${sizeStr}</td>
        <td style="padding: 10px 16px; border-right: 1px solid #0f172a; text-align: center; font-weight: 700;">${formattedArea}</td>
  ...
  const hospitalRecipientName = delivery.hospital_staff || 'PETUGAS RS';
          ...
          <table style="width: 100%; text-align: left; border-collapse: collapse; border: 1px solid #0f172a; font-size: 12px; font-weight: 600;">
            <thead>
              <tr style="background-color: #f1f5f9; color: #0f172a; text-transform: uppercase; font-weight: 900; border-bottom: 1px solid #0f172a; font-size: 9px; letter-spacing: 0.05em;">
                ...
                <th style="padding: 8px 16px; border-right: 1px solid #0f172a; text-align: center; width: 140px;">UKURAN (m)</th>
                <th style="padding: 8px 16px; border-right: 1px solid #0f172a; text-align: center; width: 100px;">LUAS (m²)</th>
                <th style="padding: 8px 16px; border-right: 1px solid #0f172a; text-align: center; width: 80px;">JUMLAH</th>
