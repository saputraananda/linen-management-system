          ...
          <>
            ...
            {isEditable && hasSigned && (
              <button
                type="button"
                onClick={clearCanvas}
                className="absolute bottom-2.5 right-2.5 px-2.5 py-1.5 bg-rose-50 text-rose-600 hover:bg-rose-100 font-bold text-[10px] rounded-lg transition cursor-pointer"
              >
                Hapus
              </button>
            )}
              ...
              <div className="relative w-full max-h-[160px] flex items-center justify-center">
                ...
                {isEditable && (
                  <button
                    type="button"
                    onClick={() => onChange('')}
                    className="absolute -top-1.5 -right-1.5 h-6 w-6 bg-rose-100 hover:bg-rose-200 text-rose-700 rounded-full flex items-center justify-center shadow transition active:scale-95 cursor-pointer"
                  >
...
export default function KurangKirimCustom() {
  ...
  const [deliveriesMap, setDeliveriesMap] = useState({}); // { detailId: { qtyDelivered, notes } }
  ...
  // Sync loads when tab or hospital changed
  useEffect(() => {
    if (hospitalId) {
  ...
  // Get historical SJ list
  const loadSjHistory = async () => {
    setLoadingSj(true);
    try {
      const token = localStorage.getItem('token');
  ...
  // Initialize form to process shortage
  const handleSelectTransaction = async (tx) => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        // Initialize deliveries map using row.id as key
        const initMap = {};
  ...
  const handleQtyChange = (rowId, val, maxLimit) => {
  ...
  const handleNotesChange = (rowId, val) => {
    setDeliveriesMap(prev => ({
      ...prev,
      [rowId]: {
        ...prev[rowId],
  ...
  const handleSubmitDelivery = async (e) => {
    ...
    Object.keys(deliveriesMap).forEach(rowId => {
      const deliveryObj = deliveriesMap[rowId];
      if (deliveryObj.qtyDelivered > 0) {
        const origItem = shortageDetails.find(d => d.id === parseInt(rowId));
        itemsToDeliver.push({
          id: parseInt(rowId),
          qtyDelivered: deliveryObj.qtyDelivered,
          length_cm: origItem ? origItem.length_cm : null,
          width_cm: origItem ? origItem.width_cm : null,
          area_m2: origItem ? origItem.area_m2 : null,
  ...
  const handlePrintActive = () => {
    const activeDetails = shortageDetails.map(item => {
      const deliverObj = deliveriesMap[item.id] || { qtyDelivered: 0, notes: '' };
      return {
        ...item,
  ...
  return (
    <div className="min-h-full py-6 bg-slate-50/50">
                          ...
                          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4 p-5">
                                        ...
                                        filteredEmployees.map(emp => (
                                          <button
                                            ...
                                            onClick={() => {
                                              ...
                                              setValetName(emp.full_name || emp.employee_name);
                                              setIsEmployeeDropdownOpen(false);
                                          ...
                                          >
                                            {toTitleCase(emp.full_name || emp.employee_name)}
                                          </button>
                                        ))
                                      )}
                                    </div>
                            ...
                            <div className="space-y-1.5">
                              ...
                              <input
                                ...
                                className="w-full px-3.5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl text-xs font-semibold focus:ring-4 focus:ring-[#1ea59e]/10 focus:border-[#1ea59e] transition outline-none uppercase"
                              />
                            </div>
                          </div>
                        </div>
                          ...
                          <table className="w-full text-left border-collapse">
                            <thead>
                              <tr className="bg-slate-50 text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-155">
                                ...
                                <th className="px-4 py-3.5 text-center w-28">Sisa Kurang</th>
                                <th className="px-4 py-3.5 text-center w-28">Jumlah Kirim</th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                              {shortageDetails.map((item, idx) => {
                                ...
                                const formattedArea = item.area_m2 ? `${parseFloat(item.area_m2).toFixed(2)} m²` : '—';
                                ...
                                return (
                                  <tr key={item.id} className="hover:bg-slate-50/50 text-xs font-medium text-slate-700">
                                    ...
                                    <td className="px-4 py-3 text-center">{len !== null ? len : '—'}</td>
                                    <td className="px-4 py-3 text-center">{wid !== null ? wid : '—'}</td>
                                    <td className="px-4 py-3 text-center font-bold">{formattedArea}</td>
                                    ...
                                    <td className="px-4 py-3 text-center">
                                      <div className="flex items-center justify-center">
                                        <input
                                          ...
                                          onChange={e => handleQtyChange(item.id, e.target.value, item.remaining_shortage)}
                          ...
                          <div className="space-y-2.5">
                            <div className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                className="flex-1 px-2.5 py-1.5 border border-slate-200 hover:border-slate-355 focus:border-teal-500 outline-none rounded-lg font-bold text-slate-800 transition"
                              />
                            </div>
                            <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-slate-500 w-36 tracking-wider">Form Transaksi Asal:</span>
                              <input
                                type="text"
                                disabled
                                value={selectedTx?.form_number || '—'}
                                className="flex-1 px-2.5 py-1.5 bg-slate-50 border border-slate-200 outline-none rounded-lg font-semibold text-slate-500 select-none cursor-not-allowed"
                              />
                            </div>
                          </div>
                          ...
                          <div className="space-y-2.5">
                            <div className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                onChange={e => setDeliveryDate(`${e.target.value}T${deliveryDate.split('T')[1] || '00:00'}`)}
                                className="flex-1 px-2.5 py-1.5 border border-slate-200 hover:border-slate-355 focus:border-teal-500 outline-none rounded-lg font-semibold text-slate-700 transition"
                              />
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-slate-500 w-36 tracking-wider">Jam:</span>
                              <input
                                type="time"
                                value={deliveryDate.split('T')[1] || ''}
                                onChange={e => setDeliveryDate(`${deliveryDate.split('T')[0] || ''}T${e.target.value}`)}
                                className="flex-1 px-2.5 py-1.5 border border-slate-200 hover:border-slate-355 focus:border-teal-500 outline-none rounded-lg font-semibold text-slate-700 transition"
                              />
                            </div>
                            <div className="flex items-center gap-2">
                          ...
                          <table className="w-full text-left border-collapse border border-slate-900 text-xs font-semibold">
                            <thead>
                              <tr className="bg-slate-100 text-slate-900 uppercase font-black tracking-wider border-b border-slate-900 text-[10px]">
                                ...
                                <th className="px-4 py-2.5 border-r border-slate-900 text-center w-20">P (m)</th>
                                <th className="px-4 py-2.5 border-r border-slate-900 text-center w-20">L (m)</th>
                                <th className="px-4 py-2.5 border-r border-slate-900 text-center w-24">Luas (m²)</th>
                                <th className="px-4 py-2.5 border-r border-slate-900 text-center w-28">Jumlah</th>
                      ...
                      {filteredSjList.map(sj => (
                        <div key={sj.id} className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm hover:shadow-md transition flex flex-col justify-between gap-5 relative overflow-hidden group">
                          ...
                          <div className="flex items-center justify-between bg-slate-50 border border-slate-100 p-3 rounded-2xl text-xs">
                            <div className="text-left">
                              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block">Penerima RS</span>
                              <span className="font-extrabold text-slate-700">{sj.recipient_name}</span>
                            </div>
                            <div className="text-right">
                              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block">Total Kirim</span>
                              <span className="font-extrabold text-[#126776]">{sj.total_qty_delivered} PCS</span>
                            </div>
                          </div>
      ...
      {/* ════════════════════════ DETAILED HISTORICAL SJ PREVIEW MODAL ════════════════════════ */}
      {viewingSj && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out] no-print">
          <div className="w-full max-w-4xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden max-h-[90vh] flex flex-col animate-[scaleIn_0.2s_ease-out]">
            {/* Header */}
            <div className="p-5 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
              ...
              <button
                ...
                className="flex h-8 w-8 items-center justify-center rounded-lg hover:bg-slate-200 text-slate-400 hover:text-slate-600 transition cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 bg-slate-100">
              {loadingSjDetails ? (
                <div className="py-16 flex flex-col items-center justify-center gap-2 text-slate-400">
                  <RefreshCw className="h-8 w-8 animate-spin text-[#1ea59e]" />
                  <span className="text-xs font-semibold">Memuat rincian Surat Jalan...</span>
                </div>
              ) : (
                <div id="print-document" className="max-w-[800px] mx-auto bg-white border border-slate-300 rounded-xl p-8 shadow-sm text-slate-800 font-sans">
                  {/* Document Header */}
                  <div className="flex flex-row items-center justify-between pb-4 border-b-2 border-slate-900 gap-4">
                    <div className="flex items-center gap-3">
                      <img src={ikmLogo} alt="IKM Logo" className="h-14 object-contain" />
                      <div>
                        <h4 className="text-md sm:text-lg font-bold text-slate-900 uppercase tracking-tight leading-none">PT. INTERSOLUSI KARYA MANDIRI</h4>
                        <p className="text-[10px] text-slate-500 font-semibold mt-1">Jl. Pringgondani No. 101, Cimanggis, Depok, Jawa Barat</p>
                        <p className="text-[10px] text-slate-500 font-semibold mt-0.5">HP: 08118871101 / 08161986580</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <h4 className="text-xl sm:text-2xl font-black tracking-widest text-slate-900 leading-none">SURAT JALAN</h4>
                      <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-wider">No. {viewingSj.surat_jalan_number}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 py-6 text-xs border-b border-slate-200">
                    <div className="space-y-2.5">
                      <div className="flex"><span className="font-bold text-slate-500 w-36 tracking-wider">Kepada Yth:</span> <span className="text-slate-800 font-bold">{viewingSj.recipient_name}</span></div>
                      <div className="flex"><span className="font-bold text-slate-500 w-36 tracking-wider">Tanggal Pengambilan:</span> <span className="text-slate-700 font-semibold">{viewingSj.original_pickup_date ? new Date(viewingSj.original_pickup_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : '—'}</span></div>
                      <div className="flex"><span className="font-bold text-slate-500 w-36 tracking-wider">Form Transaksi Asal:</span> <span className="text-slate-500">{viewingSj.original_form_number || '—'}</span></div>
                    </div>
                    <div className="space-y-2.5">
                      <div className="flex"><span className="font-bold text-slate-500 w-36 tracking-wider">Tanggal Pengiriman:</span> <span className="text-slate-700 font-semibold">{new Date(viewingSj.delivery_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span></div>
                      <div className="flex"><span className="font-bold text-slate-500 w-36 tracking-wider">Jam:</span> <span className="text-slate-700 font-semibold">{new Date(viewingSj.delivery_date).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }).replace(':', '.')}</span></div>
                      <div className="flex"><span className="font-bold text-slate-500 w-36 tracking-wider">No. Kendaraan:</span> <span className="text-slate-700">{viewingSj.vehicle_number || '—'}</span></div>
                    </div>
                  </div>
                  ...
                  <div className="py-6">
                    <table className="w-full text-left border-collapse border border-slate-900 text-xs font-semibold">
                      <thead>
                        <tr className="bg-slate-50 text-slate-900 uppercase font-black tracking-wider border-b border-slate-900 text-[10px]">
                          <th className="px-4 py-2.5 border-r border-slate-900 text-center w-12">No</th>
                          <th className="px-4 py-2.5 border-r border-slate-900 text-center">Nama Barang</th>
                          <th className="px-4 py-2.5 border-r border-slate-900 text-center w-20">P (m)</th>
                          <th className="px-4 py-2.5 border-r border-slate-900 text-center w-20">L (m)</th>
                        ...
                        {sjDetails.map((item, idx) => {
                          ...
                          return (
                            <tr key={item.id} className="text-slate-800">
                              ...
                              <td className="px-4 py-3 text-slate-500 font-normal italic">{item.notes || '—'}</td>
                            </tr>
                          );
                        })}
                      </tbody>
                  ...
                  <div className="grid grid-cols-2 gap-10 pt-10 text-center text-xs font-bold text-slate-900">
                    <div className="space-y-4">
                      ...
                      <div className="h-28 border border-slate-200 rounded-xl bg-slate-50/50 flex items-center justify-center relative overflow-hidden">
                        {viewingSj.signature_hospital ? (
                          <img src={viewingSj.signature_hospital} alt="Hospital staff signature" className="h-full object-contain" />
                        ) : (
                          <span className="text-[10px] text-slate-400 italic font-semibold">Tanda Tangan Belum Ada</span>
                        )}
                      </div>
                      <p className="border-t border-slate-950 pt-1.5 font-bold uppercase tracking-wide">{viewingSj.hospital_staff || viewingSj.recipient_name}</p>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="border-t border-slate-950 pt-1.5 font-bold uppercase tracking-wide">{viewingSj.valet_name}</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-end gap-3">
              ...
              <button
                ...
                disabled={loadingSjDetails}
                className="flex items-center gap-1.5 px-5 py-2 bg-[#126776] hover:bg-[#0e5562] text-white rounded-xl text-xs font-bold transition shadow-md shadow-[#126776]/10 cursor-pointer active:scale-95 disabled:bg-slate-300 disabled:cursor-not-allowed"
              >
                <Printer className="h-4 w-4" />
                <span>Cetak / PDF</span>
              </button>
