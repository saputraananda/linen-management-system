                    ...
                    {loadingHistory ? (
                        ...
                        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                            <div className="overflow-x-auto">
                                <table className="w-full text-left text-xs sm:text-sm border-collapse text-slate-600">
                                    <thead>
                                        <tr className="bg-slate-50 text-slate-400 font-bold uppercase tracking-wider text-[10px] border-b border-slate-200">
                                            ...
                                            <th className="py-3.5 px-4">No. Formulir</th>
                                            <th className="py-3.5 px-4 text-center">Tanggal</th>
                                            <th className="py-3.5 px-4">Valet IKM</th>
                                            <th className="py-3.5 px-4">Petugas RS</th>
                                            <th className="py-3.5 px-4 text-center bg-slate-50/50">Total Kotor</th>
                                            <th className="py-3.5 px-4 text-center bg-teal-50/20">Total Bersih</th>
                                        ...
                                        {transactions.map((tx, index) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="py-4 px-4 text-center bg-slate-50/30 text-slate-800">
                                                        {tx.total_qty_kotor} Pcs
                                                    </td>
                                                    <td className="py-4 px-4 text-center bg-teal-50/10 text-teal-700">
                                                        {tx.status === 'SELESAI' ? `${tx.total_qty_bersih} Pcs` : '—'}
                                                    </td>
                                                    <td className="py-4 px-4 text-center whitespace-nowrap">
                                                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase border ${tx.status === 'SELESAI'
                                                            ? hasShortage
                                                            ...
                                                            {tx.status === 'SELESAI'
                                                                ? hasShortage ? 'Kurang Kirim' : 'Selesai'
                                                                : 'Kotor'}
                                                        </span>
                                                    </td>
                                                    <td className="py-4 px-4 text-center whitespace-nowrap">
                                                    ...
                                                    <td className="py-4 px-4 text-center" onClick={(e) => e.stopPropagation()}>
                                                        <div className="flex items-center justify-center gap-1.5">
                                                            <button
                                                                ...
                                                                className="inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-xl border border-teal-200 bg-teal-50 hover:bg-teal-100 text-teal-700 text-xs font-bold transition shadow-sm active:scale-95 cursor-pointer disabled:opacity-50"
                                                                title="Unduh Excel"
                                                            >
                                                            ...
                                                            <button
                                                                ...
                                                                className="inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-xl border border-rose-200 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold transition shadow-sm active:scale-95 cursor-pointer disabled:opacity-50"
                                                                title="Unduh PDF"
                                                            >
                ...
                return (
                    <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                                    ...
                                    <div className={`grid grid-cols-1 ${editingTransaction.transaction.status === 'SELESAI' ? 'md:grid-cols-2' : ''} gap-6 bg-slate-50 p-5 rounded-2xl border border-slate-150 text-xs`}>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="grid grid-cols-2 gap-2 text-slate-600">
                                                ...
                                                <div>
                                                    <p className="font-semibold text-slate-400 uppercase text-[9px] tracking-wider">Pendamping RS (Saksi)</p>
                                        ...
                                        {editingTransaction.transaction.status === 'SELESAI' && (
                                            <div className="space-y-3 border-t md:border-t-0 md:border-l border-slate-200 md:pl-6">
                                                ...
                                                <div className="grid grid-cols-2 gap-2 text-slate-600">
                                                    ...
                                                    <div>
                                                        <p className="font-semibold text-slate-400 uppercase text-[9px] tracking-wider">Pendamping RS (Saksi)</p>
                                                        <p className="font-bold text-slate-855 mt-0.5">{editingTransaction.transaction.hospital_assistant_delivery || '—'}</p>
                                                    </div>
                                                </div>
                                                <div className="p-2 bg-white rounded-lg border border-slate-200">
                                    ...
                                    <div className="border border-slate-200 rounded-3xl shadow-sm overflow-hidden bg-white">
                                        <table className="w-full text-left text-xs border-collapse">
                                            ...
                                            <tbody className="divide-y divide-slate-150 text-slate-700 font-semibold">
                                                {editingTransaction.details.map((item, idx) => {
                                                    ...
                                                    return (
                                                        <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                                                            ...
                                                            <td className="py-3 px-4 text-slate-800 font-bold">{getLinenDisplayName(item)}</td>
                                                            <td className="py-3 px-4 text-center text-slate-700 font-semibold">{item.length_cm !== null ? `${parseFloat(item.length_cm).toFixed(1)}m` : '—'}</td>
                                                            <td className="py-3 px-4 text-center text-slate-700 font-semibold">{item.width_cm !== null ? `${parseFloat(item.width_cm).toFixed(1)}m` : '—'}</td>
                                                            <td className="py-3 px-4 text-center text-slate-700 font-bold">{item.area_m2 !== null ? `${parseFloat(item.area_m2).toFixed(2)} m²` : '—'}</td>
                                                            <td className="py-3 px-4 text-center bg-slate-100/30 text-slate-800 font-bold">{kotor}</td>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-200">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="grid grid-cols-3 gap-3">
                                                <div className="border border-slate-200 rounded-xl p-3 bg-white text-center flex flex-col justify-between h-[160px] shadow-sm">
                                                    <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider block">Valet IKM</span>
                                                    <div className="flex-1 flex items-center justify-center p-1 bg-slate-50 rounded border border-slate-100 my-2 overflow-hidden">
                                                        {editingTransaction.transaction.signature_valet_pickup ? (
                                                            <img src={editingTransaction.transaction.signature_valet_pickup} alt="Valet Signature" className="max-h-[70px] object-contain" />
                                                        ) : (
                                                            <span className="text-[9px] text-slate-350 italic">Belum TTD</span>
                                                        )}
                                                    </div>
                                                    <span className="text-[9px] font-bold text-slate-700 truncate">({toTitleCase(editingTransaction.transaction.user_pickup_name)})</span>
                                                </div>
                                                <div className="border border-slate-200 rounded-xl p-3 bg-white text-center flex flex-col justify-between h-[160px] shadow-sm">
                                                    ...
                                                    <div className="flex-1 flex items-center justify-center p-1 bg-slate-50 rounded border border-slate-100 my-2 overflow-hidden">
                                                        {editingTransaction.transaction.signature_hospital_pickup ? (
                                                            <img src={editingTransaction.transaction.signature_hospital_pickup} alt="Hospital Staff Signature" className="max-h-[70px] object-contain" />
                                                        ) : (
                                                            <span className="text-[9px] text-slate-350 italic">Belum TTD</span>
                                                        )}
                                                    </div>
                                                    <span className="text-[9px] font-bold text-slate-700 truncate">({toTitleCase(editingTransaction.transaction.hospital_staff_pickup || '')})</span>
                                                </div>
                                                <div className="border border-slate-200 rounded-xl p-3 bg-white text-center flex flex-col justify-between h-[160px] shadow-sm">
                                                    ...
                                                    <div className="flex-1 flex items-center justify-center p-1 bg-slate-50 rounded border border-slate-100 my-2 overflow-hidden">
                                                        {editingTransaction.transaction.signature_assistant_pickup ? (
                                                            <img src={editingTransaction.transaction.signature_assistant_pickup} alt="Perawat Signature" className="max-h-[70px] object-contain" />
                                                        ) : (
                                                            <span className="text-[9px] text-slate-350 italic">Belum TTD</span>
                                                        )}
                                                    </div>
                                                    <span className="text-[9px] font-bold text-slate-700 truncate">({toTitleCase(editingTransaction.transaction.hospital_assistant_pickup || '')})</span>
                                                </div>
                                            </div>
                                        </div>
                                                    ...
                                                    <div className="border border-slate-200 rounded-xl p-3 bg-white text-center flex flex-col justify-between h-[160px] shadow-sm">
                                                        ...
                                                        <div className="flex-1 flex items-center justify-center p-1 bg-slate-50 rounded border border-slate-100 my-2 overflow-hidden">
                                                            {editingTransaction.transaction.signature_valet_delivery ? (
                                                                <img src={editingTransaction.transaction.signature_valet_delivery} alt="Valet Signature" className="max-h-[70px] object-contain" />
                                                            ) : (
                                                                <span className="text-[9px] text-slate-350 italic">Belum TTD</span>
                                                            )}
                                                        </div>
                                                        <span className="text-[9px] font-bold text-slate-700 truncate">({toTitleCase(editingTransaction.transaction.user_delivery_name || '')})</span>
                                                    </div>
                                                    <div className="border border-slate-200 rounded-xl p-3 bg-white text-center flex flex-col justify-between h-[160px] shadow-sm">
                                                        <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider block">Petugas RS</span>
                                                        <div className="flex-1 flex items-center justify-center p-1 bg-slate-50 rounded border border-slate-100 my-2 overflow-hidden">
                                                            {editingTransaction.transaction.signature_hospital_delivery ? (
                                                                <img src={editingTransaction.transaction.signature_hospital_delivery} alt="Hospital Staff Signature" className="max-h-[70px] object-contain" />
                                                            ) : (
                                                                <span className="text-[9px] text-slate-350 italic">Belum TTD</span>
                                                            )}
                                                        </div>
                                                        <span className="text-[9px] font-bold text-slate-700 truncate">({toTitleCase(editingTransaction.transaction.hospital_staff_delivery || '')})</span>
                                                    </div>
                                                    <div className="border border-slate-200 rounded-xl p-3 bg-white text-center flex flex-col justify-between h-[160px] shadow-sm">
                                                        <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider block">Perawat RS</span>
                                                        <div className="flex-1 flex items-center justify-center p-1 bg-slate-50 rounded border border-slate-100 my-2 overflow-hidden">
                                                            {editingTransaction.transaction.signature_assistant_delivery ? (
                                                                <img src={editingTransaction.transaction.signature_assistant_delivery} alt="Perawat Signature" className="max-h-[70px] object-contain" />
                                                            ) : (
                                                                <span className="text-[9px] text-slate-350 italic">Belum TTD</span>
                                                            )}
                                                        </div>
                                                        <span className="text-[9px] font-bold text-slate-700 truncate">({toTitleCase(editingTransaction.transaction.hospital_assistant_delivery || '')})</span>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="bg-slate-50 border border-slate-200 rounded-2xl flex flex-col items-center justify-center h-[160px] text-slate-400 p-4 text-center">
                                                    <Info className="h-6 w-6 text-slate-350 mb-1.5" />
                                                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Penerima Bersih Belum Berlangsung</p>
                                                    <p className="text-[9px] text-slate-400 mt-1 font-semibold max-w-[200px]">Valet IKM belum memproses dan mengantarkan linen bersih kembali ke Rumah Sakit.</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                            ...
                            {/* ======================= TAB 2: SURAT JALAN (KURANG KIRIM) ======================= */}
                            {detailTab === 'surat_jalan' && (
                                <div className="space-y-6">
                                    {(!editingTransaction.deliveries || editingTransaction.deliveries.length === 0) ? (
                                        editingTransaction.transaction.status !== 'SELESAI' ? (
                                            <div className="bg-slate-50 border border-slate-200 rounded-3xl p-8 text-center max-w-lg mx-auto flex flex-col items-center gap-3">
                                                <div className="p-3 bg-amber-50 rounded-2xl border border-amber-100">
                                                    <Clock className="h-8 w-8 text-amber-500" />
                                                </div>
                                                <div>
                                                    <h3 className="font-bold text-slate-800 text-sm">Menunggu Pengiriman Linen Bersih</h3>
                                                    <p className="text-xs text-slate-500 max-w-md mx-auto mt-1.5 leading-relaxed font-semibold">
                                                        Kurang kirim baru dapat diproses setelah transaksi kotor ini diselesaikan (tahap pengiriman bersih selesai) jika terdapat kekurangan jumlah pengiriman.
                                                    </p>
                                                </div>
                                            </div>
                                        ) : (
                                            <div className="bg-emerald-50/50 border border-emerald-150 rounded-3xl p-8 text-center max-w-lg mx-auto flex flex-col items-center gap-3">
                                                ...
                                                <div className="space-y-5 bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-sm">
                                                    ...
                                                    {loadingSjDetails ? (
                                                        <div className="py-12 text-center text-slate-400 font-semibold">
                                                            <RefreshCw className="h-6 w-6 animate-spin mx-auto text-teal-550 mb-2" />
                                                                ...
                                                                <table className="w-full text-left text-xs border-collapse">
                                                                    <thead>
                                                                        <tr className="bg-slate-50 text-slate-400 font-bold uppercase tracking-wider text-[10px] border-b border-slate-200">
                                                                            ...
                                                                            <th className="py-2.5 px-3 text-center">Luas</th>
                                                                    ...
                                                                    <tbody className="divide-y divide-slate-150 font-semibold text-slate-700">
                                                                        {sjDetails.map((item, idx) => {
                                                                            ...
                                                                            const len = item.length_cm !== null ? parseFloat(item.length_cm).toFixed(1) : (item.lengthCm ? parseFloat(item.lengthCm).toFixed(1) : null);
                                                                            const wid = item.width_cm !== null ? parseFloat(item.width_cm).toFixed(1) : (item.widthCm ? parseFloat(item.widthCm).toFixed(1) : null);
                                                                            const sizeStr = (len && wid) ? `${len}m x ${wid}m` : '—';
                                                                            const area = item.area_m2 !== null ? parseFloat(item.area_m2).toFixed(2) : (item.areaM2 ? parseFloat(item.areaM2).toFixed(2) : null);
                                                                            const formattedArea = area > 0 ? `${area} m²` : '—';

                                                                            return (
                                                                                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                                                                                    <td className="py-2.5 px-3 text-center text-slate-400">{idx + 1}</td>
                                                                                    <td className="py-2.5 px-3 text-slate-800 font-bold">{getLinenDisplayName(item)}</td>
                                                                                    <td className="py-2.5 px-3 text-center text-slate-750 font-semibold">{sizeStr}</td>
                                                                                    <td className="py-2.5 px-3 text-center text-slate-700 font-bold">{formattedArea}</td>
